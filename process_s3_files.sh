...
FILES=$(aws s3 ls --endpoint-url $AWS_S3_ENDPOINT s3://$BUCKET/$BPATH/ | sort | tail -n $NUM_FILES | awk '{ print $4 }')
URL_LIST="[]"
while IFS= read -r FILE; do
    echo Processing $FILE
    # set acl to $ACL
    aws s3api put-object-acl --endpoint-url $AWS_S3_ENDPOINT --bucket $BUCKET --key $BPATH/$FILE --acl $ACL
